...
window.I18N = {
  de: {
    ...
    skipauthLabel: "Auth überspringen (Diagnose)",
    skipauthTitle: "Auth überspringen (Diagnose)",
    skipauthHelp: "Nur zum Testen bei einem Scooter, dessen eigene Auth scheitert (Fehler 255). Entsperre den Scooter zuerst mit der echten NAVEE-App, dann verbinde hier mit gesetztem Haken. Die Seite baut dann keinen eigenen Auth-Handshake auf, sondern schickt die Befehle direkt. So sehen wir im Log den echten Fehlercode des Scooters je Befehl (Byte 5 der RX-Antwort) und wissen, ob eine zweite Verbindung ohne eigene Auth überhaupt akzeptiert wird.",
    logSkipAuth: "Auth übersprungen (Diagnose): Befehle werden direkt gesendet. Achte im Log auf den Fehlercode der RX-Antwort je Befehl (Byte 5, 0 = ok).",
  ...
  en: {
    ...
    skipauthLabel: "Skip auth (diagnostic)",
    skipauthTitle: "Skip auth (diagnostic)",
    skipauthHelp: "Only for testing a scooter whose own auth fails (error 255). First unlock the scooter with the real NAVEE app, then connect here with this ticked. The page then builds no auth handshake of its own and sends commands directly. That way the log shows the scooter's real error code per command (byte 5 of the RX reply), so we learn whether a second connection is accepted at all without its own auth.",
    logSkipAuth: "Auth skipped (diagnostic): commands are sent directly. Watch the log for the error code of the RX reply per command (byte 5, 0 = ok).",
