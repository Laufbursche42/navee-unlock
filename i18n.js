...
window.I18N = {
  de: {
    ...
    accountHelp: "Beim Verbinden schickt die App im Auth-Handshake (0x30) die numerische userId deines NAVEE-Kontos mit. Ein bereits gebundener Scooter akzeptiert AUSSCHLIESSLICH genau diese Zahl und lehnt jede andere mit Fehler 255 ab, noch bevor er eine Challenge schickt. Wichtig: Das ist NICHT die alphanumerische Navee-ID aus dem Konto-Screen, sondern eine reine Zahl, die die App nirgends anzeigt. So kommst du dran: (1) einen HTTPS-Mitschnitt der App machen (z. B. mit einem Proxy) und in der Login-Antwort das Feld data.userId ablesen; (2) alternativ die App-Einstellungsdatei auslesen (Schlüssel userId), das braucht Root oder adb; (3) am robustesten: den echten 0x30-Auth-Frame der App mit einem BLE-Sniffer oder dem Android-HCI-Log mitschneiden und ihn unten ins Feld Auth-Frame einfügen. Eine leere oder zufällige ID funktioniert nur bei einem Scooter, der noch nie mit der App gebunden war.",
    ...
    skipauthLabel: "Auth überspringen (Scooter schon per App entsperrt)",
    skipauthTitle: "Auth überspringen (experimentell)",
    skipauthHelp: "Nur zum Ausprobieren. Wenn du deine numerische userId nicht hast, kannst du den Scooter zuerst mit der echten NAVEE-App entsperren (die App macht die Auth) und danach ohne die App weiterarbeiten, solange der Scooter eingeschaltet bleibt. Mit dieser Option baut die Seite keinen eigenen Auth-Handshake auf, sondern schickt die Befehle direkt. Ob der Scooter Schreibbefehle einer zweiten, nicht selbst angemeldeten Verbindung annimmt, hängt von seiner Firmware ab, deshalb kann es funktionieren oder auch nicht. Am Log siehst du je Befehl den Fehlercode. Der saubere Weg bleibt die echte userId.",
    logSkipAuth: "Auth übersprungen: Die Seite nimmt an, dass der Scooter bereits per offizieller App entsperrt ist und schickt Befehle direkt. Ob er sie annimmt, zeigt der Fehlercode je Antwort.",
    logUidRange: "Hinweis: Die Konto-userId ist eine 32-Bit-Zahl (maximal 2147483647, also höchstens 10 Stellen). Deine Eingabe ist zu groß und daher nicht die echte userId. Am robustesten: den echten 0x30-Auth-Frame der App mitschneiden und ins Feld Auth-Frame einfügen.",
  ...
  en: {
    ...
    accountHelp: "When connecting, the app sends your NAVEE account's numeric userId in the auth handshake (0x30). A scooter that is already bound accepts ONLY that exact number and rejects any other with error 255, before it even sends a challenge. Important: this is NOT the alphanumeric Navee ID from the account screen, but a plain number the app never shows. How to get it: (1) capture the app's HTTPS traffic (e.g. with a proxy) and read the field data.userId in the login response; (2) or read the app's settings file (key userId), which needs root or adb; (3) most robust: capture the app's real 0x30 auth frame with a BLE sniffer or the Android HCI log and paste it into the Auth frame field below. An empty or random id only works on a scooter that was never bound to the app.",
    authhexTitle: "Auth frame (hex)",
    ...
    skipauthLabel: "Skip auth (scooter already unlocked via the app)",
    skipauthTitle: "Skip auth (experimental)",
    skipauthHelp: "For experimenting only. If you do not have your numeric userId, you can first unlock the scooter with the real NAVEE app (the app does the auth) and then keep working without the app while the scooter stays powered on. With this option the page builds no auth handshake of its own and just sends the commands directly. Whether the scooter accepts writes from a second, not-itself-authenticated connection depends on its firmware, so it may or may not work. The log shows the error code per command. The clean route is still the real userId.",
    logSkipAuth: "Auth skipped: the page assumes the scooter is already unlocked via the official app and sends commands directly. The error code per reply shows whether it accepts them.",
    logUidRange: "Note: the account userId is a 32-bit number (max 2147483647, so at most 10 digits). Your input is too large to be the real userId. Most robust: capture the app's real 0x30 auth frame and paste it into the Auth frame field.",
