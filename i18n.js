...
window.I18N = {
  de: {
    ...
    tuHint: "Entsperren sendet den flash-freien Gang-4-Befehl (BLE 0x58 = 4). Der Meter kommandiert dann die im SKU festgelegte Höchstgeschwindigkeit, die der Controller auf bis zu 50,8 km/h freigibt. Kein Flashen, keine bleibende Änderung, kein Brick-Risiko. Die Geschwindigkeit ist fest, nicht einstellbar - 50,8 km/h ist die Obergrenze der Firmware.",
    tuBehavior: "Verhalten am Gerät: Das Display bleibt in D, die Boost-Taste ist ohne Funktion. Ein Wechsel in Modus S beziehungsweise ein Neustart hebt den Trick auf plus stellt die normalen Modi wieder her - also je Fahrt neu senden. Nur am eigenen Fahrzeug auf privatem Grund.",
    smTitle: "Speed-Entsperren: unterstützte Modelle",
    smNote: "Der Gang-4-Trick ist über die ganze Firmware-Kette code-belegt (Meter plus Controller geprüft).",
    ...
    rawHint: "Sendet die eingegebenen Hex-Bytes 1:1 an die Write-Characteristic (komplettes Frame inklusive 55 AA ... plus Tail). Für Reverse Engineering.",
  ...
  en: {
    ...
    tuHint: "Unlock sends the flash-free gear-4 command (BLE 0x58 = 4). The meter then commands the SKU top speed, which the controller allows up to 50.8 km/h. No flashing, no persistent change, no brick risk. The speed is fixed, not adjustable - 50.8 km/h is the firmware ceiling.",
    tuBehavior: "Behaviour on the device: the display stays in D and the boost button does nothing. Switching to mode S or a reboot cancels the trick and restores the normal modes - so resend each ride. Own vehicle on private ground only.",
    smTitle: "Speed unlock: supported models",
    smNote: "The gear-4 trick is proven across the whole firmware chain (meter plus controller checked).",
    ...
    rawHint: "Sends the entered hex bytes verbatim to the write characteristic (full frame including 55 AA ... plus tail). For reverse engineering.",
