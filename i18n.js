...
window.I18N = {
  de: {
    ...
    accountHelp: "Beim Verbinden schickt die App im Auth-Handshake (0x30) die numerische userId deines NAVEE-Kontos mit. Ein gebundener Scooter akzeptiert nur genau diese Zahl und lehnt jede andere mit Fehler 255 ab. Wichtig: Das ist NICHT die alphanumerische Navee-ID aus dem Konto-Screen, sondern eine reine Zahl mit höchstens 10 Stellen, die die App nirgends anzeigt. Einfachster Weg direkt auf dem Handy, ohne Root oder Jailbreak (die App nutzt kein Cert-Pinning): Android -> die App PCAPdroid installieren (kein Root), ihr Zertifikat aktivieren, Aufnahme starten, dich in der NAVEE-App einloggen; im Login-Aufruf an lj.naveetech.com steht in der Antwort das Feld userId. iPhone -> eine On-Device-HTTPS-App wie Http Catcher oder Stream installieren, ihr Profil plus Zertifikat unter Einstellungen aktivieren (volles Vertrauen), Aufnahme starten, in der NAVEE-App einloggen; dort dieselbe userId ablesen. Tipp: Du kannst den ganzen kopierten Antworttext einfach hier ins Feld einfügen, wir ziehen die userId automatisch heraus. Eine leere oder zufällige ID funktioniert nur bei einem Scooter, der noch nie mit der App gebunden war.",
  ...
  en: {
    ...
    accountHelp: "When connecting, the app sends your NAVEE account's numeric userId in the auth handshake (0x30). A bound scooter accepts only that exact number and rejects any other with error 255. Important: this is NOT the alphanumeric Navee ID from the account screen, but a plain number of at most 10 digits that the app never shows. Easiest way right on the phone, no root or jailbreak (the app has no cert pinning): Android -> install the PCAPdroid app (no root), enable its certificate, start capture, log into the NAVEE app; the login call to lj.naveetech.com has the field userId in its response. iPhone -> install an on-device HTTPS app like Http Catcher or Stream, enable its profile plus certificate in Settings (full trust), start capture, log into the NAVEE app; read the same userId there. Tip: you can paste the whole copied response text into this field and we extract the userId automatically. An empty or random id only works on a scooter that was never bound to the app.",
    authhexTitle: "Auth frame (hex)",
